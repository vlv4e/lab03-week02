...
game.difficulty = [1,2,3,4,5,6,7,8,9 ];
...
game.party.splice (4,0,  { number: 149, name: 'Dragonite', type: 'dragon', hp: 91, starter: false },
{ number: 150, name: 'Mewtwo', type: 'psychic', hp: 106, starter: false },
{ number: 151, name: 'Mew', type: 'psychic', hp: 100, starter: false }
);
console.log(game.party)
...
*/
function evolveStarter(party, starterIndex) {
    const evolutions = {
    ...
    const evolvedPokemonIndex = evolutions[party[starterIndex].id];
    if (evolvedPokemonIndex) {
        party.splice(starterIndex, 1, pokemon[evolvedPokemonIndex]);
    }
}
...
*/
function printStarterPokemons(pokemonArray) {
    const starters = pokemonArray.filter(pokemon => 
        [1, 4, 7, 25].includes(pokemon.id)
    );
...
function completeGymsAbove8(gyms) {
    gyms.forEach(gym => {
        if (gym.difficulty < 8) {
            gym.completed = true;
        }
